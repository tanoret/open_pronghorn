...
The script also includes published chromium diffusivity correlations for
solution-treated SUS316 / SUS316L from Mizouchi et al. (2004) so the
Hastelloy N fit can be compared with stainless-steel values.
"""
...
# ---------------------------------------------------------------------------
# 2. Constants and Arrhenius helpers
# ---------------------------------------------------------------------------
...
def arrhenius_to_celsius(value):
    """Convert 1000/T [K^-1] to temperature [degC]."""
    value = np.asarray(value, dtype=float)
    return 1000.0 / value - 273.15


def celsius_to_arrhenius(value):
    """Convert temperature [degC] to 1000/T [K^-1]."""
    value = np.asarray(value, dtype=float)
    return 1000.0 / (value + 273.15)


# ---------------------------------------------------------------------------
# 3. Fit ln(D) = ln(D0) - Q/(R*T) for INOR-8 / Hastelloy N
# ---------------------------------------------------------------------------
...
def D_bulk_inor8(temperature_k):
    """Bulk chromium diffusivity in INOR-8 / Hastelloy N [cm^2/s]."""
    return diffusivity_arrhenius(temperature_k, D0, Q_j_mol)


# ---------------------------------------------------------------------------
# 4. Published chromium diffusivity correlations for SUS316 / SUS316L
# ---------------------------------------------------------------------------
# Source:
...
#
# All pre-exponential factors below are expressed in cm^2/s.
#
# General form:
#
#     D(T) = D0 * exp(-Q / (R*T))
...
def make_hastelloy_n_plot(output_directory):
    """Create the DeVan-data Arrhenius plot and fitted Hastelloy N correlation."""
    arrhenius_x = 1000.0 / temperature_k

    temperature_line_k = np.linspace(
        temperature_k.min(),
        temperature_k.max(),
        500,
    )
    arrhenius_line_x = 1000.0 / temperature_line_k
    log10_line = np.log10(D_bulk_inor8(temperature_line_k))
...
def make_bulk_comparison_plot(output_directory):
    """Compare Hastelloy N and SUS316 bulk chromium diffusivity correlations."""
    ...
    log10_D_bulk_inor8 = np.log10(D_bulk_inor8(temperature_plot_k))
    log10_D_bulk_sus316 = np.log10(D_bulk_sus316(temperature_plot_k))
    ...
    png_path = output_directory / "chromium_bulk_diffusivity_inor8_sus316.png"
    pdf_path = output_directory / "chromium_bulk_diffusivity_inor8_sus316.pdf"
...
def main():
    ...
    print_fit_summary()
    make_hastelloy_n_plot(output_directory)
    make_bulk_comparison_plot(output_directory)
