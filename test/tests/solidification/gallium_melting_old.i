...
# k_solid = ${fparse 60.66 - 0.183*T_solidus + 6.03e-4 * T_solidus^2- 7.136e-7 * T_solidus^3}
# k_liquid = ${fparse 15.70 + 0.031*T_solidus + 2.97e-5 * T_solidus^2}
# cp_solid = ${fparse -9858.85 + 64.57*T_solidus - 0.1014 * T_solidus^2}
cp_liquid = ${fparse 828.32 - 3.056*T_solidus + 0.0079 * T_solidus^2 - 9.13e-6 * T_solidus^3}
L = 80160
alpha_b = 8.26e-5
T_solidus = 29.78
...
[FunctorMaterials]
  ...
  [rho_l]
    ...
    functor_names = 'T'
    functor_symbols = 'T'
    expression = '6273.98 - 0.605*${T_solidus} + 4.82e-5 * ${T_solidus} ^2'
    property_name = 'rho_l'
  []
  [rho_s]
    type = ParsedFunctorMaterial
  ...
  [cp_l]
    ...
    functor_names = 'T'
    functor_symbols = 'T'
    expression = '828.32 - 3.056*${T_solidus} + 0.0079 * ${T_solidus}^2 - 9.13e-6 * ${T_solidus}^3'
    property_name = 'cp_l'
  []
  [cp_s]
    type = ParsedFunctorMaterial
    functor_names = 'T'
    functor_symbols = 'T'
    expression = '-9858.85 + 64.57*${T_solidus} - 0.1014 * ${T_solidus}^2'
    property_name = 'cp_s'
  []
  [mu_l]
    type = ParsedFunctorMaterial
    functor_names = 'T'
    functor_symbols = 'T'
    expression = '0.0156 - 1.053e-4 * ${T_solidus} + 3.198e-7 * ${T_solidus}^2 - 5.005e-10 * ${T_solidus}^3'
    property_name = 'mu_l'
  []
